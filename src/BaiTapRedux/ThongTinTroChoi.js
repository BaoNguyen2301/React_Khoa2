...
class ThongTinTroChoi extends Component {
    render() {
        return (
            <div>
                <div>BẠN CHỌN: <span className='text-danger'>{this.props.taiXiu ? 'TÀI' : 'XỈU'}</span></div>
                <div>BÀN THẮNG: <span className='text-success'>{this.props.soBanThang}</span></div>
                <div>TỔNG SỐ BÀN CHƠI: <span className='text-primary'>{this.props.tongSoBanChoi}</span></div>
            </div>
        )
    }
}
